            ...
            for (auto j = 0u; j < shape[1]; ++j) {
                for (auto k = 0u; k < shape[2]; ++k) {
                    for (auto l = 0u; l < shape[3]; ++l) {
                        ...
                        T var = others.data[(i >= others.shape[0] ? 0 : i) * others.shape[1] * others.shape[2] * others.shape[3] +
                                                (j >= others.shape[1] ? 0 : j) * others.shape[2] * others.shape[3] +
                                                (k >= others.shape[2] ? 0 : k) * others.shape[3] +
                                                (l >= others.shape[3] ? 0 : l)];
